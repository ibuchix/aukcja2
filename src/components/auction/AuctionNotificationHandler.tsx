...
export const AuctionNotificationHandler = ({ dealerId }: AuctionNotificationHandlerProps) => {
  ...
  const sendAuctionOutcomeEmail = async (
    ...
    const subject = isWinner 
      ? `Congratulations! You won the auction for ${carDetails.title}`
      : `Auction Result: ${carDetails.title}`;

    const html = isWinner
      ? `
        <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
          <h1 style="color: #0ea5e9;">Congratulations!</h1>
          <p>You've won the auction for: <strong>${carDetails.title}</strong></p>
      ...
      : `
        <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
          ...
          <p>The auction for <strong>${carDetails.title}</strong> has ended.</p>
      ...
      if (bidDealerId === dealerId && !notifiedBids.has(bidId)) {
        ...
        if (status === 'won') {
          ...
          toast({
            ...
            description: `You won the auction for ${carData.title} with a bid of $${amount.toLocaleString()}`,
            variant: "default",
          });
          ...
          toast({
            ...
            description: `Your bid of $${amount.toLocaleString()} for ${carData.title} was not the winning bid`,
            variant: "default",
          });
  ...
  useEffect(() => {
    const handleAuctionEnded = async (event: CustomEvent) => {
      ...
      try {
        ...
        toast({
          title: "Auction Ended",
          description: `The auction for ${carData.title} has ended. Results will be available shortly.`,
          variant: "default",
        });
      } catch (error) {
