...
-- This ensures consistency with our new enum structure
...
BEGIN
  ...
  ELSIF NOW() >= p_start_time AND NOW() <= p_end_time THEN
    RETURN 'active';  -- CHANGED: Use 'active' instead of 'running'
  ELSE
    RETURN 'completed';
  END IF;
END;
...
-- Also update the place_bid function to check for 'active' status
...
BEGIN
    ...
    -- CHANGED: Check for 'active' instead of 'running'
