-- Update cron job to run every 10 minutes instead of every 2 minutes
SELECT cron.alter_job(29, schedule := '*/10 * * * *');

-- Also update the SECURITY DEFINER function to set payment_status to 'awaiting_seller_decision' by default
-- This ensures new dealer_won_vehicles records await seller approval before payment
...
BEGIN
    ...
    LOOP
        ...
        IF original_amount >= auction_record.reserve_price THEN
            ...
            -- Create dealer won vehicle record with awaiting_seller_decision status
            INSERT INTO dealer_won_vehicles (
                ...
                platform_fee_amount,
                'awaiting_seller_decision',  -- Default to awaiting seller decision
                auction_record.make,
            ...
            RAISE LOG 'Created dealer_won_vehicles record for car % with winning amount % (status: awaiting_seller_decision)', 
