...
-- Recreate with corrected proxy bidding logic
...
BEGIN
    ...
    FOR auction_record IN 
        ...
        WHERE c.is_auction = true
          ...
          AND c.auction_end_time > now_timestamp - INTERVAL '30 days'
    ...
    LOOP
        ...
        -- CRITICAL CHANGE: Determine winning amount based on reserve price
        IF original_amount >= auction_record.reserve_price THEN
            -- Reserve price MET: Apply proxy bidding logic
            IF second_highest_amount IS NOT NULL THEN
                IF (original_amount - second_highest_amount) > 250 THEN
                    ...
                    RAISE LOG 'Reserve MET - Proxy bidding: reducing % to % (second highest + 250)', 
                        original_amount, winning_amount;
                ELSE
                    winning_amount := original_amount;
                    RAISE LOG 'Reserve MET - Proxy bidding: keeping original amount % (difference <= 250)', 
                        original_amount;
                END IF;
            ELSE
                winning_amount := original_amount;
                RAISE LOG 'Reserve MET - Proxy bidding: keeping original amount % (no second bid)', 
                    original_amount;
            END IF;
        ELSE
            -- Reserve price NOT MET: NO proxy logic, dealer pays exact bid
            winning_amount := original_amount;
            RAISE LOG 'Reserve NOT MET - No proxy logic: dealer wins at exact bid % (reserve: %)', 
        ...
        RAISE LOG 'Platform fee for winning amount %: % PLN', winning_amount, platform_fee_amount;
        
        -- Update car as sold (ALWAYS create a winner if there are bids)
        UPDATE cars 
        ...
        -- Create dealer won vehicle record (ALWAYS, regardless of reserve)
        ...
        RAISE LOG 'Created dealer_won_vehicles record: car_id=%, winning_amount=%, original_amount=%, reserve=%, status=awaiting_seller_decision', 
            auction_record.id, winning_amount, original_amount, auction_record.reserve_price;
