...
-- This is critical for the place_bid function to work

ALTER TYPE auction_schedule_status ADD VALUE IF NOT EXISTS 'active';

-- Update our functions to handle the correct status values
-- Keep 'running' for backward compatibility but also support 'active'
...
BEGIN
  ...
  ELSIF NOW() >= p_start_time AND NOW() <= p_end_time THEN
    RETURN 'running';  -- Keep using 'running' for now since that's what the schedule uses
  ELSE
    RETURN 'completed';
  END IF;
END;
...
-- Update place_bid function to accept both 'running' and 'active' statuses
...
BEGIN
    ...
    -- Auto-correct status if there's a mismatch - but be careful with enum values
    IF v_schedule_record.status::text != v_auction_timing_status THEN
        -- Only update if the new status is a valid enum value
        IF v_auction_timing_status IN ('scheduled', 'running', 'completed', 'cancelled') THEN
    ...
    -- Check if auction is currently running (accepting bids)
    -- Accept both 'running' and 'active' as valid bidding states
    IF v_auction_timing_status NOT IN ('running', 'active') THEN
        INSERT INTO system_logs (
            log_type, 
            message, 
            error_message,
