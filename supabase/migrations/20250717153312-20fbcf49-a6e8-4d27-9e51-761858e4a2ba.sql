-- Fix the update_auction_status function to handle both starting and ending auctions
-- AND manually fix the current Octavia auction

-- First, manually fix the Octavia auction that's stuck in 'scheduled' status
UPDATE auction_schedules 
SET status = 'running',
    last_status_change = NOW(),
    updated_at = NOW()
WHERE car_id = '7a1b25df-9f74-4971-8ad9-160e57cbad3c' 
  AND status = 'scheduled' 
  AND start_time <= NOW();

-- Ensure the car's auction status is active
UPDATE cars 
...
-- Now fix the update_auction_status function to handle BOTH starting and ending auctions
